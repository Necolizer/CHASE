...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, compute_flops=False, dataset = 'NTU'):
        ...
        self.l1 = TCN_GCN_unit(in_channels, base_channels, A, residual=False, adaptive=adaptive, att=False, CoM=CoM, dataset=self.dataset)
        self.l2 = TCN_GCN_unit(base_channels, base_channels, A, adaptive=adaptive, CoM=CoM, dataset=self.dataset)
        self.l3 = TCN_GCN_unit(base_channels, base_channels, A, adaptive=adaptive, CoM=CoM, dataset=self.dataset)
        self.l4 = TCN_GCN_unit(base_channels, base_channels, A, adaptive=adaptive, CoM=CoM, dataset=self.dataset)
        self.l5 = TCN_GCN_unit(base_channels, base_channels*2, A, stride=2, adaptive=adaptive, CoM=CoM, dataset=self.dataset)
        self.l6 = TCN_GCN_unit(base_channels*2, base_channels*2, A, adaptive=adaptive, CoM=CoM, dataset=self.dataset)
        self.l7 = TCN_GCN_unit(base_channels*2, base_channels*2, A, adaptive=adaptive, CoM=CoM, dataset=self.dataset)
        self.l8 = TCN_GCN_unit(base_channels*2, base_channels*4, A, stride=2, adaptive=adaptive, CoM=CoM, dataset=self.dataset)
        self.l9 = TCN_GCN_unit(base_channels*4, base_channels*4, A, adaptive=adaptive, CoM=CoM, dataset=self.dataset)
        self.l10 = TCN_GCN_unit(base_channels*4, base_channels*4, A, adaptive=adaptive, CoM=CoM, dataset=self.dataset)
...
class Model_GROUP(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, compute_flops=False, num_class_person=5, dataset = 'CAD'):
        ...
        self.fc_person = nn.Linear(base_channels*4, num_class_person)
    ...
    def forward(self, x):
        ...
        if self.training:
            return (self.fc(x), self.fc_person(xm))
        else:
            return self.fc(x)
        
class Model_GROUP_VOL(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, compute_flops=False, num_class_person=5, dataset = 'CAD'):
        super(Model_GROUP_VOL, self).__init__()
        ...
        self.fc = nn.Linear(base_channels*4*2, num_class)
        nn.init.normal_(self.fc.weight, 0, math.sqrt(2. / num_class))

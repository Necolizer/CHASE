...
def get_parser():
    ...
    parser = argparse.ArgumentParser(description='Skeleton-based Group Activity Recgnition')
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch, (data, label, label_person, sample) in enumerate(tqdm(self.data_loader['train'], desc="Training", ncols=100)):
            self.global_step += 1
            with torch.no_grad():
                data = data.float().cuda(self.output_device)
                label = label.long().cuda(self.output_device)
                label_person = label_person.long().cuda(self.output_device)

            # forward
            output = self.model(data)
            if isinstance(output, tuple):
                if len(output) == 4:
                    loss = sum([self.loss((group, person, output[2], output[3]), label, label_person) for (group, person) in zip(output[0], output[1])])
                    output = sum([out for out in output[0]])
                else:
                    loss = sum([self.loss((group, person), label, label_person) for (group, person) in zip(output[0], output[1])])
                    output = sum([out for out in output[0]])
            else:
                loss = sum([self.loss(out, label, label_person) for out in output])
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            label_list = []
            pred_list = []
            for batch, (data, label, label_person, sample) in enumerate(tqdm(self.data_loader[ln], desc="Evaluating", ncols=100)):

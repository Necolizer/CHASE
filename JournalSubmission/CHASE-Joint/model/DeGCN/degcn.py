...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, k=8, eta=4, num_stream=2, 
                 graph=None, graph_args=dict(), in_channels=3, drop_out=0, base_frame = 64):
...
class Model_GROUP(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, k=8, eta=4, num_stream=2, 
                 graph=None, graph_args=dict(), in_channels=3, drop_out=0, base_frame = 64, num_class_person=5):
        ...
        self.fc_person = nn.ModuleList([nn.Linear(base_channel*4, num_class_person) for _ in range(self.num_stream)])
        
        for fc in self.fc_person:
            nn.init.normal_(fc.weight, 0, math.sqrt(2. / num_class_person))
    ...
    def forward(self, x):
        ...
        out = []
        out_person = []
        for stream, fc, fc_person in zip(self.streams, self.fc, self.fc_person):
            x = x_
            x = stream(x)
            c_new = x.size(1)
            x = x.view(N, M, c_new, -1)
            ...
            x = self.drop_out(x)
            out.append(fc(x))
            out_person.append(fc_person(xm))

        if self.training:
            return (out, out_person)
        else:
            return out